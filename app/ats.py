...
try:
    ...
    _SKILLS = SkillNormalizer()          # ML=머신러닝, 백엔드=서버개발 등 동의어(§6 결정론 층)
except Exception:
    _SKILLS = None
...
def _hit(kw: str, text_lower: str, en_tokens: set[str], kr_tokens: set[str]) -> bool:
    ...
    if k in text_lower:
        return True
    # 스킬 동의어(ML=머신러닝·백엔드=서버개발 등) — 그룹 어느 형태든 이력서에 있으면 hit
    if _SKILLS is not None:
        for v in _SKILLS.variants(k):
            if len(v) >= 2 and v in text_lower:
                return True
