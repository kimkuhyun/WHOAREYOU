...
# 카카오 Local(주소·키워드) API는 KA 헤더 필수(2024~). os/web + URL-encode된 origin이면
# 도메인 등록 무관하게 통과(실측). 없으면 401 "KA Header is required".
KA_HEADER = "sdk/1.0.0 os/web lang/ko-KR origin/" + urllib.parse.quote("http://localhost", safe="")
...
async def geocode_address(query: str, rest_key: str) -> GeocodeResult:
    ...
    headers = {"Authorization": f"KakaoAK {rest_key}", "KA": KA_HEADER}
